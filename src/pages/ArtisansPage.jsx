...
const ArtisansPage = () => {
    ...
    return (
        <main>
            <h1 className="fw-bold fs-3 ps-5">{categoryName ? `Liste des artisans de la catégorie ${categoryName} :` : `Résultats pour "${searchTerm}"`}</h1>
            {filteredArtisans.length === 0 ? (
                <p className="ms-5 mt-3">Aucun artisan ne correspond à votre recherche.</p>
            ) : (
                <div className="row artisans">
                    {filteredArtisans.map(artisan => (
                        <div key={artisan.id} className="artisan-card position-relative pt-3 mt-5 col-4">
                            ...
                            <Link to={`/artisan/${artisan.id}`} className='px-3 py-1 border-0 rounded-pill mb-4 button text-decoration-none'>Voir la fiche <img src={buttonIcon} alt="icone redirection" className='mb-1'/></Link>
                        </div>
                    ))}
                </div>
            )}
